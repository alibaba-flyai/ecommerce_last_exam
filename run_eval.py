...
def eval_instance(instance, cfg):
    ...
    t0 = time.time()
    container_name = f"eval_{instance_id[:12]}_{int(t0) % 10000}"
    ...
    try:
        ...
        code, stdout, stderr = docker_exec(
            container_name, "cd /app && bash tests/test.sh",
...
def cmd_run(args):
    ...
    print(f"Running {len(instances)} instances (concurrency={cfg['runner']['concurrency']})")
    ...
    if concurrency <= 1:
        for i, inst in enumerate(instances):
            ...
            print(f"[{i+1}/{len(instances)}] {inst['instance_id']}: {r_str} ({result.get('duration_sec',0):.0f}s)", flush=True)
    else:
        with ThreadPoolExecutor(max_workers=concurrency) as ex:
            futs = {ex.submit(eval_instance, inst, cfg): inst for inst in instances}
            for i, fut in enumerate(as_completed(futs)):
                ...
                print(f"[{i+1}/{len(instances)}] {result['instance_id']}: {r_str}", flush=True)

    _print_summary(results, cfg, time.time() - t0)
...
def cmd_report(args):
    ...
    summary = {
        ...
        "completed": len(valid),
        "failed": len(failed),
        "avg_reward": round(sum(r["reward"] for r in valid) / len(valid), 4) if valid else 0,
        "by_domain": {},
        "failure_breakdown": {},
    }
...
def cmd_submit(args):
    ...
    print(f"  scores.jsonl   — {len(results)} instances ({len(valid)} valid, {len(failed)} failed)")
    print(f"  summary.json   — avg_reward={avg_reward:.4f}")
    print()
    print("Next steps:")
